...
def proto_library(tags = [], **kwargs):
    ...
    native.cc_library(
        ...
        linkstatic = True,
        alwayslink = True,
        srcs = ["//%s:%s_clean" % (native.package_name(), cc_proto_rule_name,)], 
    ...
    native.cc_binary(
        ...
        deps = ["@com_google_protobuf//:protobuf"] + external_cc_deps + ["//%s:%s" % (native.package_name(), cc_lib_rule_name)],
    )

    native.cc_library(
        name = cc_wrap_rule_name,
        srcs = [":%s" % cc_bin_rule_name],
        deps = ["@com_google_protobuf//:protobuf"] + external_cc_deps + ["//%s:%s_clean" % (native.package_name(), cc_proto_rule_name,)],
        alwayslink = True,
        tags = ["exclude"],
...
def fix_proto_cc_binary_wrap(name, deps, **kwargs):
    ...
    native.cc_binary(**dict(kwargs, name=name, deps=[name + "_filter", "//:hdrs"]))

def fix_proto_cc_library_wrap(name, deps, **kwargs):
    parsed_deps, apollo_src_deps = parse_select(deps)
    filter_proto_library(name=name + "_filter", deps=parsed_deps, apollo_src_deps=apollo_src_deps)
    native.cc_library(**dict(kwargs, name=name, deps=[name + "_filter", "//:hdrs"]))

def fix_proto_cc_test_wrap(name, deps, **kwargs):
    parsed_deps, apollo_src_deps = parse_select(deps)
    filter_proto_library(name=name + "_filter", deps=parsed_deps, apollo_src_deps=apollo_src_deps)
    native.cc_test(**dict(kwargs, name=name, deps=[name + "_filter", "//:hdrs"]))
